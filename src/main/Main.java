...
public class Main {
	...
	static final String rightfiledirectoryPath ="F:\\results\\generated\\mix\\all\\GENERATED";
	static final String wrongfiledirectoryPath = "F:\\results\\generated\\mix\\all\\UNGENERATED";
	...
	public static void main(String[] args) throws IOException {
		...
		pw.println("@attribute REAK_STATEMENT numeric\n");
		//11
		...
		File directory;
		for(int j=0;j<2;j++){
			if(j==0)directoryPath=rightfiledirectoryPath;//まず正解
			else directoryPath=wrongfiledirectoryPath;
			...
			for(File file:fileList){
				...
				for(int i=1;i<=85;i++){//これで１ファイル分
					if(i<85){
						...
						if(j==0)pw.print("TRUE");//自動生成
						else pw.print("FALSE");//非自動生成
					}
				}
				pw.println("");
